...
def animate(data, face_data, model_type, save_path=None, subplots=None):
    ...
    for n in range(len(connections)):
        ...
        if (frame[branch[0]][0] and frame[branch[0]][1]) != 0 and (frame[branch[1]][0] and frame[branch[1]][1]) != 0:
            lines.append([(frame[branch[0]][0], frame[branch[0]][1]), (frame[branch[1]][0], frame[branch[1]][1])])
            # creates a tuple of values where we pull 2 of the joint points and their x and y values from the frame
        ...
        for j in range(branch[1], branch[2]):
            if (frame[j][0] and frame[j][1]) != 0 and (frame[j + 1][0] and frame[j + 1][1]) != 0:
